...
const getStatusBadge = (status: PaymentStatus) => {
    switch (status) {
        case PaymentStatus.Paid:
            return <span className="text-green-700 border-2 border-green-600 bg-green-50 px-2 py-0.5 rounded text-[10px] font-bold tracking-wider ml-2">PAID</span>;
        case PaymentStatus.PartiallyPaid:
            return <span className="text-blue-700 border-2 border-blue-600 bg-blue-50 px-2 py-0.5 rounded text-[10px] font-bold tracking-wider ml-2">PARTIAL</span>;
        case PaymentStatus.Unpaid:
            return <span className="text-red-700 border-2 border-red-600 bg-red-50 px-2 py-0.5 rounded text-[10px] font-bold tracking-wider ml-2">UNPAID</span>;
        default:
            return null;
    }
};
...
const InvoicePDFLayout: React.FC<InvoicePDFLayoutProps> = ({ invoice, quotation }) => {
    ...
    const isIntraState = invoice.billingState?.trim().toLowerCase() === 'kerala';
    ...
    const paymentInstructions = localStorage.getItem('crm_payment_instructions') ||
    ...
    const billingAddress = invoice.customer.billingAddress || invoice.customer.address;
    const shippingAddress = invoice.customer.shippingAddress || invoice.customer.address;
        ...
        <div className="bg-white text-gray-800 font-sans flex flex-col box-border w-[210mm] min-h-[297mm]">
            ...
            <header className="flex w-full mb-8 pdf-item-row overflow-hidden">
                ...
                <div className="w-[40%] bg-[#005b9e] text-white pt-10 pr-8 pl-8 pb-8 flex flex-col justify-center items-end rounded-bl-[60px] overflow-hidden">
                    <h1 className="text-4xl font-extrabold tracking-wide uppercase mb-4 text-right break-words w-full">Invoice</h1>
                    <div className="space-y-2 text-right">
                        <div>
                            <span className="block text-blue-200 text-xs font-semibold uppercase tracking-wider">Invoice #</span>
                            <span className="block text-lg font-bold">{invoice.id}</span>
                        </div>
                        <div>
            ...
            <div className="px-12 flex-grow flex flex-col">
                ...
                <section className="grid grid-cols-2 gap-12 mb-10 items-start pdf-item-row">
                    ...
                    <div>
                        ...
                        <div className="text-sm space-y-1">
                            ...
                            <p className="text-gray-600 mt-1"><span className="font-medium text-gray-800">Email:</span> {invoice.customer.email}</p>
                            <p className="text-gray-600"><span className="font-medium text-gray-800">Phone:</span> {invoice.customer.phone}</p>
                            {invoice.billingState && (
                                <p className="text-gray-600 mt-1"><span className="font-medium text-gray-800">State:</span> {invoice.billingState}</p>
                            )}
                        </div>
                    </div>
                    ...
                    {/* Ship To */}
                    <div>
                        <h3 className="text-xs font-bold text-[#005b9e] uppercase tracking-wider mb-2 border-b-2 border-[#005b9e]/20 pb-1 flex items-center justify-between">
                            <span>Ship To</span>
                ...
                <div className="grid grid-cols-2 gap-12 mt-auto pt-8 border-t border-gray-200 pdf-item-row">
                    ...
                    <div>
                         ...
                         {paymentInstructions ? (
                             <div className="text-xs text-gray-600 mb-6 bg-gray-50 p-4 rounded-md whitespace-pre-line font-mono">
                                {paymentInstructions}
                             </div>
                         ) : (
                    ...
                    <div className="flex flex-col">
                            ...
                            {discountRate > 0 && (
                             ...
                             {deliveryCharges > 0 && (
                                <div className="flex justify-between text-sm bg-purple-50 p-2 rounded border-l-4 border-purple-200">
                                    ...
                                    {/* Display Base Amount for Delivery since tax is in Output GST */}
                        ...
                        <div className="flex justify-between items-center py-4 px-2">
                            ...
                            <span className="text-3xl font-extrabold text-[#005b9e]">₹{invoice.totalAmount.toLocaleString('en-IN', {minimumFractionDigits: 0, maximumFractionDigits: 0})}</span>
                        </div>
